...
import com.xyys.memorytext.util.MemoryManager;
import com.xyys.memorytext.util.PromptManager;
import com.xyys.memorytext.util.SoftMap;
import com.xyys.memorytext.view.Hall;
...
public class MiddleManager extends Observable {
	...
	private void changeTitleAndBottom() {
		...
		// 通知：
		// 广播：多个应用
		// 为中间容器的变动增加了监听——观察者设计模式 Observer

		// ①将中间容器变成被观察的对象 extends Observable
		// ②标题和底部导航变成观察者   implements Observer
		// ③建立观察者和被观察者之间的关系（标题和底部导航添加到观察者的容器里面） 在活动初始化时建立关系 mainactivity中
